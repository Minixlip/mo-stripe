...
export async function POST(request: NextRequest) {
  ...
  const upstreamResponse = await fetch(`${AUTH_API_URL}/login`, {
    method: 'POST',
    headers: {
  ...
  if (!upstreamResponse.ok) {
    return NextResponse.json(
      { error: getErrorMessage(upstreamPayload, 'Login failed.') },
  ...
  if (!token) {
    return NextResponse.json(
      { error: 'Authentication succeeded but no session cookie was issued.' },
      { status: 502 },
    );
  }

  const response = NextResponse.json(
    upstreamPayload ?? { message: 'Successful login' },
    { status: upstreamResponse.status },
  );
