...
import { depositIntoAccount } from '../services/account.service.js';
...
export const createDeposit = async (req: Request, res: Response) => {
  ...
  const result = await depositIntoAccount(auth.userId, safeInput.data.amount, {
    key: idempotencyKey.key,
    requestHash: createIdempotencyRequestHash(
      ACCOUNT_MUTATION_OPERATIONS.DEPOSIT,
      safeInput.data,
    ),
  });
